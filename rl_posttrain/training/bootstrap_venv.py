...
uv.lock (392KB, pins alpamayo_r1 and cosmos-rl to git revisions). We just
replicate the recipe README's own two-step `uv sync` exactly:

    uv venv --python 3.12 <venv_dir>
    uv sync --active --no-install-package flash-attn   # torch etc. first
    uv sync --active                                    # then flash-attn,
                                                         # which needs torch
                                                         # at build time and
                                                         # is configured with
                                                         # no-build-isolation
...
def ensure_recipe_venv(venv_dir: str, recipe_dir: str) -> str:
    ...
    _run(sync_cmd + ["--no-install-package", "flash-attn"], cwd=recipe_dir, env={**env, "VIRTUAL_ENV": venv_dir})
    _run(sync_cmd, cwd=recipe_dir, env={**env, "VIRTUAL_ENV": venv_dir})
