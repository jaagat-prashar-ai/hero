...
launch_one() {
  local suffix="$1" top_k="$2" min_passes="$3" score_mode="$4"
  unshare -Ur -m bash -c '
  ...
  ' bash "${hosts_file}" python3 lilypad/launch.py \
    ...
    -o runtime_environment.constant_environment_variables.CODE_REWARD_VERIFY_MIN_PASSES "${min_passes}" \
...
cd "${repo_root}"
failures=0
launch_one twotier-top1-rank-v1 top1 all rank || failures=$((failures + 1))
launch_one majority-top3-raw-v1 top3 majority raw || failures=$((failures + 1))
launch_one majority-top3-rank-v1 top3 majority rank || failures=$((failures + 1))
exit "${failures}"
