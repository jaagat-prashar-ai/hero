...
BASE_RUN_TAG="${CLIPGEN_BASE_RUN_TAG:-full1050-v1}"
  ...
  repair)
    ...
    for shard in $(seq "${START_SHARD}" "${END_SHARD}"); do
      python3 "${REPO_ROOT}/lilypad/launch.py" "${TEMPLATE}" \
        ...
        -o workload_variant_config.training_fn_config.selection_base_report_s3_key \
          "code_as_a_reward/clipgen/offline_rewards/${BASE_RUN_TAG}-shard${shard}/report.json" \
