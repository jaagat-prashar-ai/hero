...
"""
masked_model_a1.py — CoT-masked Alpamayo 1 (R1) for open-loop semantic-action alignment.

Port of masking/masked_model.py (MaskedAlpamayo1_5) to the original
Alpamayo 1 release (NVlabs/alpamayo, nvidia/Alpamayo-R1-10B). R1's rollout is
structurally identical to 1.5's — 1.5 merely factored the eos-offset and
expert-mask construction into `_find_eos_offset` /
`_build_expert_pos_ids_and_attn_mask` static helpers (and added prefix_mask
support), so this fork carries verbatim copies of those two 1.5 statics and
otherwise reuses the 1.5 fork's analysis code unchanged.

- Imported:
    - alpamayo_r1.models.alpamayo_r1.AlpamayoR1, ExpertLogitsProcessor
    - alpamayo_r1.models.token_utils.{to_special_token, StopAfterEOS,
...
reproduces the stock `AlpamayoR1.sample_trajectories_from_data_with_vlm_rollout`
...
class MaskedAlpamayoR1(AlpamayoR1):
    ...
    # in its own rollout, plus optional prefix_mask support).
    # ------------------------------------------------------------------ #
    @staticmethod
